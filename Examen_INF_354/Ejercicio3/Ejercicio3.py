...
df = pd.read_csv('/content/drive/MyDrive/dataset/grafo.csv')
pesos = np.asarray(df)
pesos
...
for i in range(10000):
    ...
    new_poblacion_set = poblacion_(progenitor_list) 
    mutacion_pop = mutacion_poblacion(new_poblacion_set)
print("la mejor solucion")
print(mejor_solucion)
df = pd.DataFrame(mejor_solucion)
df.to_csv('ejercicio3.csv', index=False, header=False,sep=',')